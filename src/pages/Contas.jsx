...
export default function Contas({
  ...
  function salvar() {
    ...
    onSave({
      ...
      centro_custo_id: form.centro_id,
      status: form.status,
  ...
  return (
    <section className="card">
      <div className="sectionHeader">
        ...
        <button className="plus" onClick={() => setOpen(true)}>
          +
        </button>
      </div>
      ...
      <div className="filters">
        <select
          ...
          onChange={(e) =>
            setFiltros({ ...filtros, centro: e.target.value })
          }
        >
        ...
        <div className="modalOverlay" onClick={() => setOpen(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, centro_id: e.target.value })
              }
            >
              {centros.map((c) => (
                <option key={c.id} value={c.id}>
      ...
      {contas.map((conta) => (
        <article className="accountCard" key={conta.id}>
          <div>
            ...
            {/* 🔥 CORREÇÃO DO CENTRO */}
            <p>{conta.df_centros_custo?.nome || "—"}</p>

            <small>
              Vencimento: {dateBR(conta.data_vencimento)}
            </small>
          </div>
          ...
          <span
            className={`status ${
              conta.status === "Pago" ? "pago" : "aberto"
            }`}
          >
            {conta.status}
          </span>

          <button onClick={() => onTogglePaid(conta)}>
            Pago/Aberto
          </button>

          <button className="danger" onClick={() => onDelete(conta)}>
            Excluir
          </button>
        </article>
      ))}
