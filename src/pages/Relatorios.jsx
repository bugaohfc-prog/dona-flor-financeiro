...
import AIFinancialCopilot from '../components/reports/AIFinancialCopilot'
...
export default function Relatorios({ voltar, empresaId, mostrarAviso }) {
  ...
  const aiCopilot = useMemo(() => {
    const nivel = scoreSaude < 45 ? 'Crítico' : scoreSaude < 75 ? 'Atenção' : 'Estável'
    const cor = scoreSaude < 45 ? '#dc3545' : scoreSaude < 75 ? '#f59f00' : '#12b886'
    const centroCritico = principalCentro
      ? `${principalCentro.nome} concentra ${formatarPercentual(principalCentro.percentual)} do total filtrado.`
      : 'Nenhum centro dominante encontrado no filtro atual.'
    const maiorRisco = totalVencido > 0
      ? `O maior risco atual está em contas vencidas, somando ${formatarValor(totalVencido)}.`
      : principalCentro?.percentual >= 60
        ? `O maior risco é concentração: ${centroCritico}`
        : 'Não há risco crítico dominante no filtro atual.'
    const prioridadePrincipal = totalVencido > 0
      ? 'Priorize a baixa ou renegociação das contas vencidas antes de expandir novas despesas.'
      : contasSemCentro > 0
        ? 'Classifique as contas sem centro para melhorar leitura, forecast e auditoria.'
        : 'Mantenha acompanhamento semanal dos indicadores e da meta mensal.'
    const melhoriaPrevisao = contasSemCentro > 0
      ? `Classificar ${contasSemCentro} conta(s) sem centro aumenta a confiabilidade do forecast.`
      : 'A previsão já possui boa base de classificação. O próximo ganho vem de histórico mensal mais longo.'
    const narrativa = `${nivel === 'Crítico' ? 'O cenário financeiro exige ação imediata.' : nivel === 'Atenção' ? 'O cenário financeiro pede acompanhamento próximo.' : 'O cenário financeiro está controlado.'} ${maiorRisco} Para os próximos 90 dias, o forecast aponta ${formatarValor(camadaPreditiva.previsao90)} com risco projetado de ${formatarPercentual(camadaPreditiva.riscoProjetado)}. ${centroCritico}`
    const prioridades = [
      { titulo: totalVencido > 0 ? 'Reduzir vencidos' : 'Manter disciplina de pagamentos', descricao: totalVencido > 0 ? `Resolver ${formatarValor(totalVencido)} vencidos reduz o risco imediatamente.` : 'Sem vencidos críticos no filtro atual.' },
      { titulo: contasSemCentro > 0 ? 'Classificar centros' : 'Preservar qualidade dos dados', descricao: contasSemCentro > 0 ? `${contasSemCentro} conta(s) ainda estão sem centro de custo.` : 'Todas as contas filtradas possuem centro de custo.' },
      { titulo: principalCentro?.percentual >= 60 ? 'Desconcentrar custos' : 'Monitorar concentração', descricao: centroCritico }
    ]
    const recomendacoes = [
      prioridadePrincipal,
      principalCentro?.percentual >= 60 ? `Revise despesas do centro ${principalCentro.nome} antes do próximo fechamento.` : 'Continue monitorando o ranking de centros para prevenir concentração.',
      camadaPreditiva.riscoProjetado >= 65 ? 'Crie uma rotina semanal de revisão de vencidos e forecast.' : 'Use a meta mensal para transformar a previsão em plano de ação.'
    ]
    return { nivel, cor, score: scoreSaude, riscoProjetado: camadaPreditiva.riscoProjetado, forecast90: camadaPreditiva.previsao90, narrativa, prioridades, recomendacoes, maiorRisco, prioridadePrincipal, melhoriaPrevisao, centroCritico }
  }, [scoreSaude, principalCentro, totalVencido, contasSemCentro, camadaPreditiva])
  ...
  function exportarExcel() {
    const sheets = [
      ...
      {
        name: 'Copilot IA 11.5',
        rows: [
          ['Indicador', 'Valor', 'Observação'],
          ['Nível Copilot', aiCopilot.nivel, 'Classificação executiva'],
          ['Narrativa executiva', aiCopilot.narrativa, 'Resumo automático'],
          ['Maior risco', aiCopilot.maiorRisco, 'Pergunta rápida'],
          ['Prioridade principal', aiCopilot.prioridadePrincipal, 'Ação sugerida'],
          [],
          ['Prioridades inteligentes'],
          ...aiCopilot.prioridades.map((item, index) => [index + 1, item.titulo, item.descricao]),
          [],
          ['Recomendações IA'],
          ...aiCopilot.recomendacoes.map((item, index) => [index + 1, item])
        ]
      }
    ]
    ...
    <div className="relatorios-page" style={styles.page}>
      ...
      <header className="no-print" style={styles.hero}>
        <div>
          ...
          <p style={styles.descricaoTela}>Fase 11.5: AI Financial Copilot com narrativa executiva, recomendações e prioridades inteligentes.</p>
        </div>
      ...
      <section className="no-print relatorio-sticky-filtros" style={styles.filtrosBox}>
        ...
        <div style={styles.filtrosGrid}>
          ...
          <select style={styles.input} value={visaoExecutiva} onChange={(e) => setVisaoExecutiva(e.target.value)}>
            ...
            <option value="copilot">Copilot IA 11.5</option>
          </select>
      ...
      <section style={styles.advancedPanel}>
        <div style={styles.widgetHeader}>
          <div>
            ...
            <p style={styles.muted}>DRE gerencial, gráficos executivos, inteligência 11.3, predição 11.4 e Copilot IA 11.5.</p>
          </div>
