...
export default function Relatorios({ voltar }) {
  ...
  if (percentualClassificacao < 40 && contasFiltradas.length > 0) {
    insights.push({
      tipo: 'critico',
      texto: `Qualidade dos dados crítica: apenas ${formatarPercentual(percentualClassificacao)} das contas filtradas possuem centro de custo. Antes de tomar decisões, classifique as despesas sem centro.`
    })
  } else if (percentualClassificacao < 80 && contasFiltradas.length > 0) {
    insights.push({
      tipo: 'ação',
      texto: `Qualidade dos dados em atenção: ${contasSemCentro} conta(s) ainda estão sem centro. Classificar essas contas melhora os relatórios.`
    })
  }
  ...
  if (temSemCentro) {
    insights.push({
      tipo: 'ação',
      texto: `Existem despesas sem centro de custo, somando ${formatarValor(semCentro?.total || 0)}. Classificar essas contas melhora a leitura gerencial.`
    })
  }
