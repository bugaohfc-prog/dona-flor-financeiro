    ...
    const html = `<!doctype html>
      <html lang="pt-BR">
        ...
        <body>
          ...
          <div class="insight"><strong>Liquidez:</strong> ${escapeHtml(copilotFinanceiro.narrative?.liquidez || '')}</div>
          <div class="insight"><strong>Concentração:</strong> ${escapeHtml(copilotFinanceiro.narrative?.concentracao || '')}</div>
          <div class="insight"><strong>Curto prazo:</strong> ${escapeHtml(copilotFinanceiro.narrative?.curtoPrazo || '')}</div>
          <table><thead><tr><th>Sinal</th><th>Severidade</th><th>Leitura</th><th>Ação</th></tr></thead><tbody>${(copilotFinanceiro.narrative?.anomalias || []).map((item) => `<tr><td>${escapeHtml(item.title)}</td><td>${escapeHtml(item.severity)}</td><td>${escapeHtml(item.description)}</td><td>${escapeHtml(item.action)}</td></tr>`).join('')}</tbody></table>
          <div class="cards">
    ...
    const sheets = [
      ...
      },
      {
        ...
        rows: [
          ...
          [],
          ['AI Narrative & Insights 11.8'],
          ['Parecer executivo avançado', copilotFinanceiro.narrative?.parecer || ''],
          ['Liquidez', copilotFinanceiro.narrative?.liquidez || ''],
          ['Concentração', copilotFinanceiro.narrative?.concentracao || ''],
          ['Curto prazo', copilotFinanceiro.narrative?.curtoPrazo || ''],
          [],
          ['Anomalias contextuais'],
          ['Tipo', 'Severidade', 'Sinal', 'Leitura', 'Ação'],
          ...(copilotFinanceiro.narrative?.anomalias || []).map((item) => [item.type, item.severity, item.title, item.description, item.action]),
          [],
    ...
    <div className="relatorios-page" style={styles.page}>
      ...
      <header className="no-print" style={styles.hero}>
        <div>
          ...
          <p style={styles.descricaoTela}>Fase 11.8: AI Narrative & Insights com parecer executivo avançado, anomalias e recomendações contextuais.</p>
        </div>
      ...
      <section style={styles.advancedPanel}>
        <div style={styles.widgetHeader}>
          <div>
            ...
            <p style={styles.muted}>DRE gerencial, gráficos executivos, tendência, multiunidade, inteligência financeira 11.3, preditiva 11.4 e Copilot IA 11.8.</p>
          </div>
        ...
        {visaoExecutiva === 'copilot' && (
          <div style={styles.advancedGrid}>
            ...
            <Widget titulo="AI Narrative & Insights 11.8" emoji="🧠" badge={`${copilotFinanceiro.narrative?.anomalias?.length || 0} sinais`} badgeColor="#0f766e">
              <p style={styles.executivoTexto}>{copilotFinanceiro.narrative?.parecer}</p>
              <div style={styles.insightList}>
                {(copilotFinanceiro.narrative?.anomalias || []).slice(0, 3).map((item, index) => (
                  <div key={`${item.title}-${index}`} style={styles.insightItem}>
                    <span style={styles.insightEmoji}>{item.severity}</span>
                    <p><strong>{item.title}</strong><br />{item.description}<br /><small>{item.action}</small></p>
                  </div>
                ))}
              </div>
            </Widget>
