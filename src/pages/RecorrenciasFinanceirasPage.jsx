...
function chaveDuplicidadeSerie(serie) {
  return [normalizarTexto(serie.descricao), Number(serie.valor || 0).toFixed(2), String(serie.dia_vencimento || ''), serie.centro_custo_id || '', serie.filial_id || ''].join('|')
}
...
function GestaoRecorrencias({ fonte, centros, filiais, styles, formatarValor, formatarData, formatarTipoRecorrencia, abrirConfirmacao, desativarSerieRecorrente, reativarSerieRecorrente }) {
  ...
  const visiveis = useMemo(() => series.filter((serie) => {
    ...
    return [serie.descricao, (centros || []).find((item) => item.id === serie.centro_custo_id)?.nome, (filiais || []).find((item) => item.id === serie.filial_id)?.nome].some((valor) => normalizarTexto(valor).includes(termo))
...
export default function RecorrenciasFinanceirasPage({ empresaId, styles, centros = [], filiais = [], formatarValor, formatarData, formatarTipoRecorrencia, navegarPara, navegarParaConta, abrirConfirmacao, desativarSerieRecorrente, reativarSerieRecorrente }) {
  ...
  return <main className="accounts-page recurring-coverage-page">
    <div className="page-title-actions accounts-page-header"><div className="accounts-page-header-copy"><span>Financeiro</span><h1>Recorrências financeiras</h1><p>Confira a cobertura por horizonte e gerencie séries sem gerar ou vincular contas automaticamente.</p></div><button type="button" onClick={() => navegarPara?.('contas')}>Voltar para Contas</button></div>
