...
export default function Relatorios({ voltar }) {
  ...
  const centrosComVencido = ranking.filter((item) => item.vencido > 0)
  const quantidadeVencidas = contasFiltradas.filter((conta) => estaVencida(conta.data_vencimento, conta.status)).length

  const insights = []

  if (principalCentro && principalCentro.percentual >= 40) {
    insights.push({
      tipo: 'atenção',
      texto: `${principalCentro.nome} concentra ${formatarPercentual(principalCentro.percentual)} do total filtrado. Vale revisar se esse peso está esperado.`
    })
  }
  ...
  if (filtroMes) {
    if (totalMesAnterior === 0 && totalGeral > 0) {
      ...
      insights.push({
        tipo: 'alta',
        texto: `O total subiu ${formatarValor(diferencaMes)} em relação a ${nomeMes(mesAnterior(filtroMes))}, variação de ${formatarPercentual(percentualMes)}.`
      })
    } else if (diferencaMes < 0) {
      insights.push({
        tipo: 'queda',
        texto: `O total caiu ${formatarValor(Math.abs(diferencaMes))} em relação a ${nomeMes(mesAnterior(filtroMes))}, redução de ${formatarPercentual(Math.abs(percentualMes))}.`
      })
        ...
        <div style={styles.comparativoGrid}>
          ...
          <div>
            ...
            <strong style={{ color: diferencaMes > 0 ? '#dc3545' : diferencaMes < 0 ? '#198754' : '#333' }}>
              {diferencaMes > 0 ? '+' : ''}{formatarValor(diferencaMes)}
            </strong>
      ...
      <section className="print-card" style={styles.cardInsights}>
        ...
        {insights.map((insight, index) => (
          <div key={index} style={styles.insightItem}>
            <span>{insight.tipo === 'risco' ? '⚠️' : insight.tipo === 'queda' ? '✅' : insight.tipo === 'alta' ? '📈' : 'ℹ️'}</span>
            <p>{insight.texto}</p>
          </div>
        ))}
      </section>
