...
export function NarrativeIntelligenceCard() {
  ...
  return (
    <section className="copilot-card copilot-narrative-card">
      <div className="copilot-card-head">
        <span>AI Narrative & Insights 11.8</span>
        <strong>{narrative.anomalias?.length || 0} sinais</strong>
      </div>
      <p className="copilot-narrative-lead">{narrative.parecer}</p>
      <div className="copilot-narrative-grid">
        <article>
          <small>Liquidez</small>
          <p>{narrative.liquidez}</p>
        </article>
        <article>
          <small>Concentração</small>
          <p>{narrative.concentracao}</p>
        </article>
        <article>
          <small>Curto prazo</small>
          <p>{narrative.curtoPrazo}</p>
        </article>
      </div>
      <div className="copilot-anomaly-list">
        {narrative.anomalias?.slice(0, 3).map((item, index) => (
